...
def bpkewed():
    ...
    s5=0
    s1=0
    with open('arrivalskewedbp.csv', 'r') as csvfile:
    ...
    with open('consumptionrateskewedbp.csv', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for row in plots:
            #t.append(math.floor(double(row[0])))
            #t.append(str(row[0]))
    ...
    #
    plt.title('Skewed Workload with binpack autoscaler and load-aware assignment')
    ax1.legend()
    ax2.legend(loc =4 )
    #
    plt.show()
...
def lagskewedbp():
    ...
    s5=0
    s1=0
    with open('lagskewedbp.csv', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for row in plots:
            #t.append(math.floor(double(row[0])))
            #t.append(str(row[0]))
